...
func (t *SimpleChaincode) getHistoryForAsset(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	fmt.Println(" ####### In getHistoryForAsset  #######")
	if len(args) < 1 {
...
func main() {
	if s := os.Getenv("STARTASSERVER"); s == "true" {
		fmt.Printf("Starting chaincode as server")
		startserver()
	} else {
		fmt.Printf("Starting chaincode as client")
...
func startserver() {
	...
	err := server.Start()
	if err != nil {
		fmt.Printf("Error starting example02 chaincode: %s", err)
	}
}
